...
//HTTPProcessor 用于处理http协议
type HTTPProcessor struct {
}

func (s *HTTPProcessor) Encode(req *Request, res *Response) {
	file, ok := res.Body.(*os.File)
	if ok {
		//如果res.Body是os.File类型
		defer file.Close()
		fileInfo, err1 := os.Lstat(req.serv.StaticFilePath + req.URI)
...
func (s *HTTPProcessor) Decode(req *Request, res *Response) {
	conn := req.Conn
